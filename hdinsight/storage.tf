...
/* additional storage account for Custom Action Scripts */

resource "azurerm_storage_account" "addl_stg" {
  name                     = "${var.prefix}addlstg"
  resource_group_name      = azurerm_resource_group.rg.name
  location                 = azurerm_resource_group.rg.location
  account_tier             = "Standard"
  account_replication_type = "LRS"
  ...
  network_rules {
    default_action = "Deny"
    ip_rules = [var.external_client_iprange]
    virtual_network_subnet_ids = [ azurerm_subnet.vm-subnet.id, azurerm_subnet.hdi-subnet.id ]
  }
...
resource "azurerm_storage_container" "addl_stg_cont" {
  name                  = "scripts"
  storage_account_name  = azurerm_storage_account.addl_stg.name
